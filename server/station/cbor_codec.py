...
def decode_detection_obj(obj: Any) -> DetectionMessage:
    ...
    spatial_flags = int(spatial.get(8, 0))
    tdoa = [int(spatial.get(i, 0)) for i in range(6)] if spatial else []
    ...
    return DetectionMessage(
        ...
        station=StationPosition(
            lat_e7=int(payload.get(0, 0)), lon_e7=int(payload.get(1, 0)),
            alt_dm=int(payload.get(2, 0)), pos_accuracy_m=20.0,
            altitude_source="gnss_msl",
        ),
        gnss=GnssStatus(
            fix_type=int(payload.get(6, 0)), satellites=int(payload.get(7, 0)),
            hdop_x100=int(payload.get(8, 9999)), pps_ok=bool(payload.get(3, False)),
            expected_time_error_us=int(payload.get(9, 1_000_000)),
            jam=bool(flags & 1), spoof=bool(flags & 2),
        ),
        classification=Classification(
            class_id=class_id, label=_CLASS_LABEL.get(class_id, f"CLASS_{class_id}"),
            confidence_u8=confidence, unknown=(class_id == 0),
        ),
        hierarchy=HierarchicalClassification(
            ...
            type_id=type_id, type_label=_TYPE_LABEL.get(type_id, f"TYPE_{type_id}"),
        ...
        single_station_estimate=SingleStationEstimate(
            height_dm=int(single.get(0, 0)), height_sigma_dm=int(single.get(1, 0)),
            speed_dmps=int(single.get(2, 0)), speed_sigma_dmps=int(single.get(3, 0)),
            confidence_u8=int(single.get(4, 0)),
            height_valid=bool(valid_flags & 0x01), speed_valid=bool(valid_flags & 0x02),
            motion_hint=_MOTION_HINT.get(int(single.get(6, 0)), "UNKNOWN"),
        ),
        features=features,
        doa=DoaEstimate(
            azimuth_cdeg=int(doa.get(0, 0)), elevation_cdeg=int(doa.get(1, 0)),
            sigma_cdeg=int(doa.get(2, 18000)), valid=bool(doa.get(3, False)),
        ),
        spatial=SpatialInfo(
            tdoa_us=tdoa,
            confidence_u8=int(spatial.get(6, 0)),
            geometry_quality_u8=int(spatial.get(7, 0)),
            valid=bool(spatial_flags & 0x01),
            upper_hemisphere=bool(spatial_flags & 0x02),
        ),
        power=PowerStatus(
            battery_pct=int(status.get(0, 0)), battery_mv=int(status.get(1, 0)),
            solar_mv=int(status.get(2, 0)), temperature_c10=int(status.get(3, 200)),
        ),
        route=RouteStatus(
