...
static void test_layout_with_stores(void) {
  ...
  const uint32_t stores = (2u + 2u + ZS_NOR_STORAGE_NRF_IMAGE_BLOCKS + ZS_NOR_STORAGE_BOOT_COUNTER_BLOCKS + ZS_NOR_STORAGE_SECRETS_BLOCKS) * blk;
  assert(v2.capacity_bytes == cap && v2.stores_partition_bytes == stores);
  assert(v2.outbox_base_address + v2.outbox_partition_bytes == v2.secrets_base_address);
  assert(v2.secrets_base_address + ZS_NOR_STORAGE_SECRETS_BLOCKS * blk == v2.boot_counter_base_address);
  assert(v2.boot_counter_base_address + blk == v2.nrf_image_base_address);
  ...
  assert(v2.secrets_base_address == v1.outbox_base_address + v1.outbox_partition_bytes - stores);
  ...
  assert(v2.secrets_base_address == 0x03f79000u && v2.boot_counter_base_address == 0x03f7b000u && v2.nrf_image_base_address == 0x03f7c000u && v2.config_base_address == 0x03ffc000u && v2.installation_base_address == 0x03ffe000u); /* W25Q512JV map, B3 + C.6 */
