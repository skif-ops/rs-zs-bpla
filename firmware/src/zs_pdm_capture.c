...
bool zs_pdm_capture_channel_lag(const zs_pdm_capture_t *c, unsigned ch, uint32_t count, int max_lag, int *lag_out) {
  static int16_t ref[4096], other[4096];
  ...
  if (!zs_audio_ring_copy_mono(c->ring, end, count, ref, 0u) || !zs_audio_ring_copy_mono(c->ring, end, count, other, ch)) return false;
  for (uint32_t i = 0u; i < count; i++) {
    energy_ref += (int64_t)ref[i] * ref[i];
    energy_other += (int64_t)other[i] * other[i];
  }
  if (energy_ref < (int64_t)count * 16 || energy_other < (int64_t)count * 16) return false; /* effectively silence */
  for (int lag = -max_lag; lag <= max_lag; lag++) {
    int64_t acc = 0;
    for (uint32_t i = (uint32_t)max_lag; i + (uint32_t)max_lag < count; i++) acc += (int64_t)ref[i] * other[(int)i + lag];
