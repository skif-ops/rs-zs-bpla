...
static bool adapter_valid(const zs_nor_command_journal_adapter_t *adapter) {
  uint64_t partition_end;
  if (!adapter || !adapter->nor || adapter->slot_count < 2u ||
      !adapter->nor->port.command || !adapter->nor->port.millis ||
      !adapter->nor->port.delay_ms ||
      adapter->nor->geometry.capacity_bytes == 0u ||
      adapter->nor->geometry.page_bytes == 0u ||
      adapter->nor->geometry.erase_bytes < ZS_COMMAND_JOURNAL_SLOT_BYTES ||
...
static bool slot_range(
    const zs_nor_command_journal_adapter_t *adapter,
    uint16_t slot,
    uint32_t offset,
    size_t size,
    uint32_t *address) {
  ...
  if (!adapter_valid(adapter) || slot >= adapter->slot_count || !address ||
      (uint64_t)offset + (uint64_t)size > ZS_COMMAND_JOURNAL_SLOT_BYTES)
    return false;
...
static bool journal_read(void *ctx, uint16_t slot, uint32_t offset,
                         uint8_t *data, size_t size) {
  zs_nor_command_journal_adapter_t *adapter = ctx;
  uint32_t address;
  return data && slot_range(adapter, slot, offset, size, &address) &&
         zs_nor_read(adapter->nor, address, data, size);
}

static bool journal_erase(void *ctx, uint16_t slot) {
  zs_nor_command_journal_adapter_t *adapter = ctx;
  uint32_t address;
  return slot_range(adapter, slot, 0u, ZS_COMMAND_JOURNAL_SLOT_BYTES,
                    &address) &&
         zs_nor_erase(adapter->nor, address,
                      adapter->nor->geometry.erase_bytes);
}

static bool journal_write(void *ctx, uint16_t slot, uint32_t offset,
...
bool zs_nor_command_journal_io_init(
  ...
  if (!adapter || !nor || !out_io || !nor->port.command ||
      ...
      nor->geometry.capacity_bytes == 0u ||
      nor->geometry.erase_bytes == 0u || nor->geometry.page_bytes == 0u ||
      slot_count < 2u ||
      nor->geometry.erase_bytes < ZS_COMMAND_JOURNAL_SLOT_BYTES ||
  ...
  *out_io = (zs_command_journal_io_t){
      adapter, slot_count, journal_read, journal_erase, journal_write};
  return true;
}
