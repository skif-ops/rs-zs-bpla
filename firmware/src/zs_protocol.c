...
static size_t encode_detection_impl(const zs_detection_t *m, uint8_t *out, size_t cap, bool include_features) {
  ...
  /* v1.1 keys remain unchanged. v1.2 adds 12/13; v1.3 full packet adds 14. */
  zs_cbor_map(&c, include_features ? 15 : 13);
  ...
  if (include_features) {
    /* Spatial diagnostics are LTE/P1 data. P0 LoRa summary stays unchanged. */
    zs_cbor_uint(&c, 14);
    zs_cbor_map(&c, 9);
    for (unsigned i = 0; i < ZS_SPATIAL_TDOA_COUNT; i++) {
      kvi(&c, i, m->spatial.tdoa_us[i]);
    }
    kvu(&c, 6, m->spatial.confidence_u8);
    kvu(&c, 7, m->spatial.geometry_quality_u8);
    kvu(&c, 8, m->spatial.flags);
  }
