...
### 2.1 3V8 modem power-integrity contract

- `3V8_MODEM` передаётся одной силовой парой: pin 1 и единственный возврат
  `GND_MODEM` pin 2, оба 18 AWG / 0.75 mm2. Параллельных контактов и remote
  sense в Rev.A нет.
- На PCB-MAIN обязательны `C36` и `C44`, KEMET `T520D107M006ATE015`, по
  `100 uF` непосредственно у ветвей BG95 VBAT_BB и VBAT_RF; они не заменяются
  общей ёмкостью на PCB-PWR.
- При representative LTE/EGPRS burst напряжение должно оставаться не ниже
  `3.3 V` на всех четырёх VBAT pads U8.
- End-to-end сопротивление RF power path при 25 C должно быть не более
  `100 mOhm`. Отдельно проверяются падение на контактах/жгуте и нагрев при
  +70 C; расчёт без физического burst-теста не закрывает gate.
- Изменение числа контактов, калибра провода, локальной ёмкости или порога
  требует межплатного ECO и повторного load-step/burst review.

### 2.2 Power-good semantics

`PWR_GOOD` на pin 7 является только статусом всегда включённой шины 3V3.
`PG_3V8` намеренно не транспортируется на PCB-MAIN: `R15` остаётся DNP, а
`TP5` служит bench/EVT точкой. Прошивка диагностирует 3V8 косвенно по состоянию
модема и INA226; прямой GPIO-сигнал `PG_3V8` потребует отдельного межплатного ECO.
...
- измерение сопротивления 3V8 RF path <=100 mOhm при 25 C и проверка
  `U8 VBAT >=3.3 V` во время LTE/EGPRS burst при nominal и +70 C;
