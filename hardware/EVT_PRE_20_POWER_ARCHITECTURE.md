...
Статус: `DRAFT / ONE EVT CANDIDATE PER FUNCTION SELECTED / NOT RUN / NOT A PRODUCTION RELEASE`
...
- солнечная панель: выбранный EVT-кандидат 80 W в пределах класса 60-80 W;
...
## 2.1. Один точный набор EVT-кандидатов

| Функция | Производитель / точный MPN | Контролируемая характеристика | До выпуска остаётся |
|---|---|---|---|
| батарея | RELiON `RB40` | 12.8 V, 40 Ah, 512 Wh, M8, IP66 | cold-soak и системная работа при заданных `-40..+70 C`, транспортные документы, capacity/DCIR/BMS sample |
| панель | SLD Tech / Solarland `SLP080S-12M` | 80 W, Vmp 18.46 V, Voc 22.52 V, Isc 4.56 A | IV, cold-Voc, изоляция, кронштейн, кабель и транспортный sample |
...
Это **выбранные EVT-кандидаты**, а не серийная AVL. На выбранной партии нельзя
смешивать альтернативные MPN. Серийная модель и возможная ёмкость 60 Ah принимаются
только после результатов EVT и управляемого изменения.
