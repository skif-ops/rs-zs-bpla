#!/usr/bin/env python3
"""Join the C7/U1 3V3 island to the C6 supply via on In3.Cu."""
...
BASE=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-C9-3V3-032/PCB-MAIN_P2_C9_3V3_032_CANDIDATE_REV_A.kicad_pcb'
BASE_DRC=BASE.parent/'drc_candidate.json'
OUT=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-C7-3V3-033'
CANDIDATE=OUT/'PCB-MAIN_P2_C7_3V3_033_CANDIDATE_REV_A.kicad_pcb'
BASE_SHA='205da6e000c30460cd9e587b95d48170170306a205051d9a76a2acffa0df7e39'
NAMESPACE=uuid.UUID('1fa3421e-638a-4772-a55d-e7b9cfa22d2b')
NET='3V3_DIGITAL'
ROUTES=(('In3.Cu',.25,((41.75,30.4),(42.5,28.85),
                         (44.2,26.65),(44.25,26.5))),)
VIAS=()
...
def run_drc():
 work=ROOT/'hardware/kicad/native/_p2_c7_3v3_033';shutil.rmtree(work,ignore_errors=True)
 ...
 try:
  ...
  (OUT/'PCB-MAIN_P2_C7_3V3_033_CANDIDATE_REV_A.kicad_pro').write_text(overlay)
...
def main():
 if '--check' in sys.argv:
  ...
  print('PCB-MAIN C7 3V3 033: PASS',record['drc']);return
 gate.deps();OUT.mkdir(parents=True,exist_ok=True);CANDIDATE.write_text(build())
 drc=run_drc();summary={'schema':'dioneya-pcb-main-c7-3v3-033-v1','base_sha256':BASE_SHA,
  'candidate_sha256':sha(CANDIDATE),'routes':len(ROUTES),'track_segments':3,
  'added_vias':0,'total_length_mm':round(sum(__import__('shapely').geometry.LineString(points).length for _,_,points in ROUTES),3),
  'capacitor_supply_return_review_b':'OPEN',
