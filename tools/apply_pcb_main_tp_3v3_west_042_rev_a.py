#!/usr/bin/env python3
"""Join the west 3V3 front island to the TP_EOL back-side pad."""
...
BASE=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-VCORE-U1-49-041/PCB-MAIN_P2_VCORE_U1_49_041_CANDIDATE_REV_A.kicad_pcb'
BASE_DRC=BASE.parent/'drc_candidate.json'
OUT=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-TP-3V3-WEST-042'
CANDIDATE=OUT/'PCB-MAIN_P2_TP_3V3_WEST_042_CANDIDATE_REV_A.kicad_pcb'
BASE_SHA='53d46280a79067be6e05a6fb10640cb76e63c1168e04b8e65730aaccdf6629d3'
NAMESPACE=uuid.UUID('529e447b-f219-4752-a8c0-20ff60c14d59')
NET='3V3_DIGITAL'
ROUTES=(('B.Cu',.3,((32.675,18.9),(34.175,20.4),(34.175,21.4),(34.375,21.6),(34.375,21.8),(35.54,23))),)
VIAS=((32.675,18.9),)
...
def check_geometry():
 ...
 for layer,width,points in ROUTES:
  ...
  assert any(n==NET and l=='B.Cu' and shape.contains(Point(*points[-1])) for n,l,shape in pads)
  assert not [(n,line.distance(c)-r-width/2) for c,r,n in holes if n!=NET and line.distance(c)-r-width/2<.25-1e-6]
 for x,y in VIAS:
  q=Point(x,y)
  assert any(shape.contains(q) for shape in front_tracks)
...
def run_drc():
 work=ROOT/'hardware/kicad/native/_p2_tp_3v3_west_042';shutil.rmtree(work,ignore_errors=True)
 ...
 try:
  ...
  (OUT/'PCB-MAIN_P2_TP_3V3_WEST_042_CANDIDATE_REV_A.kicad_pro').write_text(overlay)
...
def main():
 if '--check' in sys.argv:
  ...
  print('PCB-MAIN west TP 3V3 042: PASS',record['drc']);return
 gate.deps();OUT.mkdir(parents=True,exist_ok=True);CANDIDATE.write_text(build())
 drc=run_drc();summary={'schema':'dioneya-pcb-main-tp-3v3-west-042-v1','base_sha256':BASE_SHA,
  'candidate_sha256':sha(CANDIDATE),'routes':len(ROUTES),'track_segments':5,
  'via_size_drill_mm':[.25,.15],'total_length_mm':5.277,
  'test_pad_supply_and_mixed_domain_return_review_b':'OPEN','small_via_dfm_review':'OPEN',
