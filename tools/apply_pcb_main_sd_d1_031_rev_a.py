#!/usr/bin/env python3
"""Join SD_D1_CARD across the local 3V3 copper on B.Cu."""
...
BASE=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-C6-R14-030/PCB-MAIN_P2_C6_R14_030_CANDIDATE_REV_A.kicad_pcb'
BASE_DRC=BASE.parent/'drc_candidate.json'
OUT=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-SD-D1-031'
CANDIDATE=OUT/'PCB-MAIN_P2_SD_D1_031_CANDIDATE_REV_A.kicad_pcb'
BASE_SHA='2cddda588a3ad265fdf36331f40809c6dde9ec46fea99636874d87850652b529'
NAMESPACE=uuid.UUID('4e198537-fbe1-4693-bae6-6d92dff36624')
NET='SD_D1_CARD'
ROUTES=(('F.Cu',.15,((85.075,25.0811),(84.875,25.0811))),
        ('B.Cu',.15,((84.875,25.0811),(84.9,25.1),(85.1,24.9),
                       (85.1,24.65),(85.4,24.35),(85.45,24.35),
                       (85.5,24.3),(86.75,24.3),(86.8,24.25),
                       (86.825,24.25))))
VIAS=((84.875,25.0811),(86.825,24.25))
...
def check_geometry():
 ...
 for layer,width,points in ROUTES:
  ...
  for x,y in (points[0],points[-1]):
   assert (NET,layer,x,y) in pads or (NET,layer,x,y) in endpoints or (x,y) in VIAS
  assert not [(n,line.distance(c)-r-width/2) for c,r,n in holes if n!=NET and line.distance(c)-r-width/2<.25-1e-6]
 for x,y in VIAS:
  assert any((x,y) in points for layer,width,points in ROUTES if layer=='F.Cu') or (NET,'F.Cu',x,y) in pads
...
def run_drc():
 work=ROOT/'hardware/kicad/native/_p2_sd_d1_031';shutil.rmtree(work,ignore_errors=True)
 ...
 try:
  ...
  (OUT/'PCB-MAIN_P2_SD_D1_031_CANDIDATE_REV_A.kicad_pro').write_text(overlay)
...
def main():
 if '--check' in sys.argv:
  ...
  print('PCB-MAIN SD D1 031: PASS',record['drc']);return
 gate.deps();OUT.mkdir(parents=True,exist_ok=True);CANDIDATE.write_text(build())
 drc=run_drc();summary={'schema':'dioneya-pcb-main-sd-d1-031-v1','base_sha256':BASE_SHA,
  ...
  'sd_signal_return_review_b':'OPEN','via_in_pad_dfm_review':'OPEN',
