#!/usr/bin/env python3
"""Independent scope, geometry and comparative DRC audit for PCB-PWR 008."""
...
from generate_pcb_pwr_vbat_sys_c13_c12_routing_008_candidate_rev_a import (
    BASE,
    BASE_SHA256,
    CANDIDATE,
    ROOT,
...
GENERATOR_SHA256 = "d35bf0bb2d951e977d8c3827299732a8da607478b77609d88e3525bba7a20eca"
CANDIDATE_SHA256 = "bb4b5363c9d03daae5b0a81b9f048878aa6d0a38bcb541b24b681f1489b5e71e"
CANDIDATE_SEMANTIC_SHA256 = "1278dcdc1c752e34132af5ac6472525c5012774ff864aa2210bee40c8c2284e3"
...
def comparative_drc(base_path: Path, candidate_path: Path) -> dict:
    ...
    assert [base_unconnected, candidate_unconnected] == [107, 106], \
        "candidate 008 connectivity delta drift"
    return {
        "status": "PASS_NO_DRC_FINGERPRINT_DELTA_UNCONNECTED_107_TO_106",
...
def audit(drc_base: Path | None = None, drc_candidate: Path | None = None) -> dict:
    ...
    assert digest(Path(__file__).with_name("generate_pcb_pwr_vbat_sys_c13_c12_routing_008_candidate_rev_a.py")) == GENERATOR_SHA256
    ...
    assert len(base.traceItems) == 37 and len(candidate.traceItems) == 39
    assert candidate.traceItems[:37] == base.traceItems
    ...
    for ref, pad_number, anchor in (("C13", "1", ROUTES[0][0]),
                                     ("C12", "1", ROUTES[-1][1])):
        assert pads[(ref, pad_number)].net.name == "VBAT_SYS"
    ...
    assert math.isclose(clearance["minimum_edge_clearance_mm"], 0.875, abs_tol=1e-6)
    ...
    assert review["source_binding"]["generator_sha256"] == GENERATOR_SHA256
    assert review["source_binding"]["routing_rules_sha256"] == ROUTE_RULES_SHA256
    assert review["source_binding"]["evt_stackup_basis_sha256"] == STACKUP_SHA256
    assert review["deferred_boundary"]["application_authorized"] is False
    assert review["invariants"]["authoritative_board_modified"] is False
    assert review["routing_complete"] is False
    assert review["review_b_complete"] is False
    ...
    result = {
        ...
        "one_way_drop_4a_70c_35um_mv": round(resistance * 4000, 6),
...
if __name__ == "__main__":
    ...
    print("PCB-PWR C13-to-C12 008 candidate:", report["status"])
