#!/usr/bin/env python3
"""Connect R61 to the existing GNSS antenna-bias supply run."""
...
BASE=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-GNSS-SHORT-STATUS-046/PCB-MAIN_P2_GNSS_SHORT_STATUS_046_CANDIDATE_REV_A.kicad_pcb'
BASE_DRC=BASE.parent/'drc_candidate.json'
OUT=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-GNSS-BIAS-R61-047'
CANDIDATE=OUT/'PCB-MAIN_P2_GNSS_BIAS_R61_047_CANDIDATE_REV_A.kicad_pcb'
BASE_SHA='59d2b0f07f032ff3ec67b3c6a920b49491c427a5edd96d6a36ede60fe0d92dbc'
NAMESPACE=uuid.UUID('11915521-a5ab-41d1-9d1f-bdb06d09c7c6')
NET='GNSS_ANT_BIAS_RAW'
ROUTES=(('In3.Cu',.3,((49,51.825),(53.9,55.025),(54.0168,55.183))),)
VIAS=((49,51.825),(54.0168,55.183))
...
def run_drc():
 work=ROOT/'hardware/kicad/native/_p2_gnss_bias_r61_047';shutil.rmtree(work,ignore_errors=True)
 ...
 try:
  ...
  (OUT/'PCB-MAIN_P2_GNSS_BIAS_R61_047_CANDIDATE_REV_A.kicad_pro').write_text(overlay)
...
def main():
 if '--check' in sys.argv:
  ...
  print('PCB-MAIN GNSS bias R61 047: PASS',record['drc']);return
 gate.deps();OUT.mkdir(parents=True,exist_ok=True);CANDIDATE.write_text(build())
 drc=run_drc();summary={'schema':'dioneya-pcb-main-gnss-bias-r61-047-v1','base_sha256':BASE_SHA,
  'candidate_sha256':sha(CANDIDATE),'routes':len(ROUTES),'track_segments':2,
  'via_size_drill_mm':[.25,.15],'total_length_mm':6.049,
  'gnss_bias_current_and_rf_isolation_review_b':'OPEN','via_in_pad_dfm_review':'OPEN',
