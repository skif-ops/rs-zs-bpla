#!/usr/bin/env python3
"""Join SIM2_VDD_CONN to its existing F.Cu supply run."""
...
BASE = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-SIM-MUX-019/PCB-MAIN_P2_SIM_MUX_019_CANDIDATE_REV_A.kicad_pcb"
BASE_DRC = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-SIM-MUX-019/drc_candidate.json"
OUT = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-SIM2-VDD-020"
CANDIDATE = OUT / "PCB-MAIN_P2_SIM2_VDD_020_CANDIDATE_REV_A.kicad_pcb"
BASE_SHA = "b07416af6bf3fd4d100e5b38496a061d5a33f1325a53aeb383c1dbcc1bb0ae30"
NAMESPACE = uuid.UUID("27327570-a01a-41d0-b61b-87bba7392304")

# Net, width (mm), pad centre, legal bends, existing track endpoint (mm).
ROUTES = (
    ("SIM2_VDD_CONN", .25, ((72.695,11.5),(71.895,11.35),(62.945,12.6),(62.095,13.0083))),
)
...
def build() -> str:
    ...
    for net, width, points in ROUTES:
        for j,(a,b) in enumerate(zip(points,points[1:])):
            key = f"P2-SIM2-VDD-020|{net}|{width}|{j}|{a}|{b}"
...
def run_drc() -> dict:
    work = ROOT / "hardware/kicad/native/_p2_sim2_vdd_020"
    ...
    try:
        ...
        (OUT / "PCB-MAIN_P2_SIM2_VDD_020_CANDIDATE_REV_A.kicad_pro").write_text(overlay, encoding="utf-8")
...
def main() -> None:
    if "--check" in sys.argv:
        ...
        print("PCB-MAIN SIM2 VDD 020: PASS", record["drc"])
        return
    gate.deps()
    ...
    drc = run_drc()
    from shapely.geometry import LineString
    summary = {"schema": "dioneya-pcb-main-sim2-vdd-020-v1", "base_sha256": BASE_SHA,
