#!/usr/bin/env python3
"""Join two bounded power islands to existing F.Cu copper with L2 reference."""
...
BASE = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-3V3-015/PCB-MAIN_P2_3V3_015_CANDIDATE_REV_A.kicad_pcb"
BASE_DRC = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-3V3-015/drc_candidate.json"
OUT = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-POWER-016"
CANDIDATE = OUT / "PCB-MAIN_P2_POWER_016_CANDIDATE_REV_A.kicad_pcb"
BASE_SHA = "00b01ae33b8d6a457c084c163403520855c56c84efdd24869c5145dfae1d3dfc"
NAMESPACE = uuid.UUID("02688d42-fc9f-48e2-9952-9b2c753f1719")

# Net, width (mm), pad centre, legal bends, existing track endpoint (mm).
ROUTES = (
    ("3V3_DIGITAL", .3, ((45.675,20.75),(45.475,20.198),(45.475,19.748),
                          (46.875,18.248),(47.125,18.098),(47.675,18.148),(48.675,18.898))),
    ("1V8_MIC", .25, ((54.24,46.0),(53.76,46.0),(53.26,45.5),(52.76,45.5))),
)
...
def build() -> str:
    ...
    for net, width, points in ROUTES:
        for j,(a,b) in enumerate(zip(points,points[1:])):
            key = f"P2-POWER-016|{net}|{width}|{j}|{a}|{b}"
...
def run_drc() -> dict:
    work = ROOT / "hardware/kicad/native/_p2_power_016"
    ...
    try:
        ...
        (OUT / "PCB-MAIN_P2_POWER_016_CANDIDATE_REV_A.kicad_pro").write_text(overlay, encoding="utf-8")
...
def main() -> None:
    if "--check" in sys.argv:
        ...
        print("PCB-MAIN power islands 016: PASS", record["drc"])
        return
    gate.deps()
    ...
    drc = run_drc()
    from shapely.geometry import LineString
    summary = {"schema": "dioneya-pcb-main-power-islands-016-v1", "base_sha256": BASE_SHA,
