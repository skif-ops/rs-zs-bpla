...
def main() -> int:
    ...
    refs = {ref: record.instance for ref, record in model.symbols.items()}
    ...
    if model.pin_net("D1", by_name["K"]) != "VBAT_FUSED":
        raise RuntimeError("D1 cathode must connect to VBAT_FUSED")
    if model.pin_net("D1", by_name["A"]) != "GND_PWR":
    ...
    for ref, (net1, net2, semantic) in CAP_NETS.items():
        ...
        if model.pin_net(ref, "1") != net1 or model.pin_net(ref, "2") != net2:
    ...
    for ref, net in TP_NETS.items():
        ...
        if model.pin_net(ref, "1") != net:
            raise RuntimeError(f"{ref}: expected {net}, got {model.pin_net(ref, '1')}")
