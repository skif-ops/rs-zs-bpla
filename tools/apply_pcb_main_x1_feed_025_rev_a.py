#!/usr/bin/env python3
"""Join R1/X1 3V3 island to the existing supply trace."""
...
BASE = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-X1-SUPPLY-024/PCB-MAIN_P2_X1_SUPPLY_024_CANDIDATE_REV_A.kicad_pcb"
BASE_DRC = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-X1-SUPPLY-024/drc_candidate.json"
OUT = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-X1-FEED-025"
CANDIDATE = OUT / "PCB-MAIN_P2_X1_FEED_025_CANDIDATE_REV_A.kicad_pcb"
BASE_SHA = "b02929037204ab6e1422a890b34cdb3cf2a92059fee29f82432e8e01bfa48b9a"
NAMESPACE = uuid.UUID("890b082f-8797-40e7-b1b7-56db72388db7")

# Net, width (mm), pad centre, bend and U1 pad centre (mm).
ROUTES = (
    ("3V3_DIGITAL", .25, ((61.425,28.5),(61.525,28.048),(61.675,27.898),(63.175,27.898))),
)
...
def build() -> str:
    ...
    for net, width, points in ROUTES:
        for j,(a,b) in enumerate(zip(points,points[1:])):
            key = f"P2-X1-FEED-025|{net}|{width}|{j}|{a}|{b}"
...
def run_drc() -> dict:
    work = ROOT / "hardware/kicad/native/_p2_x1_feed_025"
    ...
    try:
        ...
        (OUT / "PCB-MAIN_P2_X1_FEED_025_CANDIDATE_REV_A.kicad_pro").write_text(overlay, encoding="utf-8")
...
def main() -> None:
    if "--check" in sys.argv:
        ...
        print("PCB-MAIN X1 feed 025: PASS", record["drc"])
        return
    gate.deps()
    OUT.mkdir(parents=True, exist_ok=True)
    CANDIDATE.write_text(build(), encoding="utf-8")
    drc = run_drc()
    summary = {"schema": "dioneya-pcb-main-x1-feed-025-v1", "base_sha256": BASE_SHA,
               "candidate_sha256": sha(CANDIDATE), "track_segments": 3, "width_mm": .25,
               "length_mm": 2.18, "drc": drc, "candidate_only_usb_u1_via_rules": True,
               "oscillator_power_integrity_review": "OPEN: 0.15 mm downstream supply branch to X1",
