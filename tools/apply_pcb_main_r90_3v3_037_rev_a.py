#!/usr/bin/env python3
"""Join the 10 kΩ SD_DET pull-up R90 to nearby 3V3 copper on B.Cu."""
...
BASE=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-SIM1-RST-036/PCB-MAIN_P2_SIM1_RST_036_CANDIDATE_REV_A.kicad_pcb'
BASE_DRC=BASE.parent/'drc_candidate.json'
OUT=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-R90-3V3-037'
CANDIDATE=OUT/'PCB-MAIN_P2_R90_3V3_037_CANDIDATE_REV_A.kicad_pcb'
BASE_SHA='844e3f7f34eda15c56eefc46bfe661bb78d5e142208a8c769076f37d509fed42'
NAMESPACE=uuid.UUID('68b77d17-e904-4ab5-8f0f-c5815bdc5a29')
NET='3V3_DIGITAL'
ROUTES=(('F.Cu',.15,((80.675,23.0),(80.325,23.25))),
        ('B.Cu',.15,((80.325,23.25),(80.01,20.5))))
VIAS=((80.325,23.25),(80.01,20.5))
...
def check_geometry():
 ...
 for x,y in VIAS:
  assert any((x,y) in points and (NET,'F.Cu',points[0][0],points[0][1]) in pads for layer,width,points in ROUTES if layer=='F.Cu') or any(t.distance(Point(x,y))<.001 for t in front_tracks)
...
def run_drc():
 work=ROOT/'hardware/kicad/native/_p2_r90_3v3_037';shutil.rmtree(work,ignore_errors=True)
 ...
 try:
  ...
  (OUT/'PCB-MAIN_P2_R90_3V3_037_CANDIDATE_REV_A.kicad_pro').write_text(overlay)
...
def main():
 if '--check' in sys.argv:
  ...
  print('PCB-MAIN R90 3V3 037: PASS',record['drc']);return
 gate.deps();OUT.mkdir(parents=True,exist_ok=True);CANDIDATE.write_text(build())
 drc=run_drc();summary={'schema':'dioneya-pcb-main-r90-3v3-037-v1','base_sha256':BASE_SHA,
  ...
  'sd_detect_pullup_power_review_b':'OPEN','small_via_dfm_review':'OPEN',
