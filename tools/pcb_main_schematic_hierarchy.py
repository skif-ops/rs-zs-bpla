#!/usr/bin/env python3
"""Resolve pins and nets in flat or hierarchical PCB-MAIN schematics.

The hierarchy materializer writes one explicit wire stub per connected pin and
uses local or hierarchical labels only at the far end of that stub.  Production
audits therefore must follow real wire connectivity instead of assuming that a
label is placed directly on a symbol pin.
"""
...
def property_value(instance, key: str) -> str:
    return next((str(item.value) for item in instance.properties
                 if str(item.key) == key), "")


def ref_of(instance) -> str:
    return property_value(instance, "Reference")
...
class HierarchicalSchematic:
    """Load one root and its direct functional child sheets."""
    ...
    def __init__(self, root_path: Path) -> None:
        ...
        for sheet in root.sheets:
            ...
            if child_path in seen:
                raise RuntimeError(f"duplicate/cyclic PCB-MAIN sheet file: {child_path}")
            if not child_path.is_file():
                raise RuntimeError(f"PCB-MAIN child sheet missing: {child_path}")
            seen.add(child_path)
        ...
        for document in self.documents:
            ...
            for instance in document.schematic.schematicSymbols:
                ...
                self.symbols[ref] = SymbolRecord(
                    document, instance, libraries[instance.libId]
                )
