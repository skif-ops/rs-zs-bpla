#!/usr/bin/env python3
"""Join the upper 3V3 F.Cu island to the TP_EOL B.Cu 3V3 run."""
...
BASE=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-CELL-DBG-RX-038/PCB-MAIN_P2_CELL_DBG_RX_038_CANDIDATE_REV_A.kicad_pcb'
BASE_DRC=BASE.parent/'drc_candidate.json'
OUT=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-TP-3V3-039'
CANDIDATE=OUT/'PCB-MAIN_P2_TP_3V3_039_CANDIDATE_REV_A.kicad_pcb'
BASE_SHA='2b3d8b12b1ff0cc4e774d99f16a011c8725c8e0ce20216d97d33de38bdc680dc'
NAMESPACE=uuid.UUID('16296b7a-13e7-4af8-892c-b98c9145dbcb')
NET='3V3_DIGITAL'
ROUTES=(('B.Cu',.15,((45.6069,20.562),(44.55,21.6),
                       (44.5,21.7),(44.34,23.5))),)
VIAS=((45.6069,20.562),)
...
def run_drc():
 work=ROOT/'hardware/kicad/native/_p2_tp_3v3_039';shutil.rmtree(work,ignore_errors=True)
 ...
 try:
  ...
  (OUT/'PCB-MAIN_P2_TP_3V3_039_CANDIDATE_REV_A.kicad_pro').write_text(overlay)
...
def main():
 if '--check' in sys.argv:
  ...
  print('PCB-MAIN TP 3V3 039: PASS',record['drc']);return
 gate.deps();OUT.mkdir(parents=True,exist_ok=True);CANDIDATE.write_text(build())
 drc=run_drc();summary={'schema':'dioneya-pcb-main-tp-3v3-039-v1','base_sha256':BASE_SHA,
  ...
  'test_pad_supply_return_review_b':'OPEN','small_via_dfm_review':'OPEN',
