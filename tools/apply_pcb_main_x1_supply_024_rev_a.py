#!/usr/bin/env python3
"""Join R1.1 and low-current X1.3 supply with a narrow F.Cu escape."""
...
BASE = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-3V3-MODEM-023/PCB-MAIN_P2_3V3_MODEM_023_CANDIDATE_REV_A.kicad_pcb"
BASE_DRC = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-3V3-MODEM-023/drc_candidate.json"
OUT = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-X1-SUPPLY-024"
CANDIDATE = OUT / "PCB-MAIN_P2_X1_SUPPLY_024_CANDIDATE_REV_A.kicad_pcb"
BASE_SHA = "11c28e2fd090a20cd6d14eb46b1f9c282f5ff5b2fbe714a041e0d4e2c5ce1a4a"
NAMESPACE = uuid.UUID("890b082f-8797-40e7-b1b7-56db72388db7")

# Net, width (mm), pad centre, bend and U1 pad centre (mm).
ROUTES = (
    ("3V3_DIGITAL", .15, ((61.425,28.5),(61.625,28.0),(61.725,27.95),
                           (62.425,27.95),(62.625,28.15),(62.575,29.15),(62.25,29.795))),
)
...
def build() -> str:
    ...
    for net, width, points in ROUTES:
        for j,(a,b) in enumerate(zip(points,points[1:])):
            key = f"P2-X1-SUPPLY-024|{net}|{width}|{j}|{a}|{b}"
...
def run_drc() -> dict:
    work = ROOT / "hardware/kicad/native/_p2_x1_supply_024"
    ...
    try:
        ...
        (OUT / "PCB-MAIN_P2_X1_SUPPLY_024_CANDIDATE_REV_A.kicad_pro").write_text(overlay, encoding="utf-8")
...
def main() -> None:
    if "--check" in sys.argv:
        ...
        print("PCB-MAIN X1 supply 024: PASS", record["drc"])
        return
    gate.deps()
    OUT.mkdir(parents=True, exist_ok=True)
    CANDIDATE.write_text(build(), encoding="utf-8")
    drc = run_drc()
    summary = {"schema": "dioneya-pcb-main-x1-supply-024-v1", "base_sha256": BASE_SHA,
               "candidate_sha256": sha(CANDIDATE), "track_segments": 6, "width_mm": .15,
               "length_mm": 3.36, "drc": drc, "candidate_only_usb_u1_via_rules": True,
               "oscillator_power_integrity_review": "OPEN: 0.15 mm supply branch to X1",
