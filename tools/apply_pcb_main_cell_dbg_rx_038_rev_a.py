#!/usr/bin/env python3
"""Join U27 CELL_DBG_RXD_TP to the existing debug via on In3.Cu."""
...
BASE=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-R90-3V3-037/PCB-MAIN_P2_R90_3V3_037_CANDIDATE_REV_A.kicad_pcb'
BASE_DRC=BASE.parent/'drc_candidate.json'
OUT=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-CELL-DBG-RX-038'
CANDIDATE=OUT/'PCB-MAIN_P2_CELL_DBG_RX_038_CANDIDATE_REV_A.kicad_pcb'
BASE_SHA='75558caa18c0d3fc3d8c3c9ff0c3e1700bb56b50821d9bcc29582a1bde5c80f9'
NAMESPACE=uuid.UUID('b5997a35-06bb-4c91-b2f6-0bfa095bd990')
NET='CELL_DBG_RXD_TP'
ROUTES=(('F.Cu',.15,((40.5825,64.5),(40.5825,64.45))),
        ('In3.Cu',.15,((40.5825,64.45),(40.05,63.6),
                         (40.05,63.1),(40.75,61.3),(40.3804,57.475))))
VIAS=((40.5825,64.45),)
...
def check_geometry():
 ...
 for x,y in VIAS:
  assert any((x,y) in points for layer,width,points in ROUTES if layer=='F.Cu')
...
def run_drc():
 work=ROOT/'hardware/kicad/native/_p2_cell_dbg_rx_038';shutil.rmtree(work,ignore_errors=True)
 ...
 try:
  ...
  (OUT/'PCB-MAIN_P2_CELL_DBG_RX_038_CANDIDATE_REV_A.kicad_pro').write_text(overlay)
...
def main():
 if '--check' in sys.argv:
  ...
  print('PCB-MAIN CELL DBG RX 038: PASS',record['drc']);return
 gate.deps();OUT.mkdir(parents=True,exist_ok=True);CANDIDATE.write_text(build())
 drc=run_drc();summary={'schema':'dioneya-pcb-main-cell-dbg-rx-038-v1','base_sha256':BASE_SHA,
  'candidate_sha256':sha(CANDIDATE),'routes':len(ROUTES),'track_segments':5,
  ...
  'debug_pdm_coupling_mixed_return_review_b':'OPEN','via_in_pad_dfm_review':'OPEN',
