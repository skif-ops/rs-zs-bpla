#!/usr/bin/env python3
"""Join U1.49 VCORE to the existing inner-layer supply via."""
...
BASE=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-FB1-INPUT-040/PCB-MAIN_P2_FB1_INPUT_040_CANDIDATE_REV_A.kicad_pcb'
BASE_DRC=BASE.parent/'drc_candidate.json'
OUT=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-VCORE-U1-49-041'
CANDIDATE=OUT/'PCB-MAIN_P2_VCORE_U1_49_041_CANDIDATE_REV_A.kicad_pcb'
BASE_SHA='0b1b7a449271a79290c6fdd51b845e29678e73d87c2ac7d8320ab00823bc5422'
NAMESPACE=uuid.UUID('0b389042-f253-4966-a43e-9083e40d943b')
NET='VCORE_1V1'
ROUTES=(('In3.Cu',.3,((57.5,37.75),(55.824,34.9762))),)
VIAS=((57.5,37.75),)
...
def check_geometry():
 ...
 pads=[];endpoints=set();existing_vias=[]
 ...
 for n,k,l,shape,raw in geo.items(b):
  if k=='via':
   ...
   if n==NET:existing_vias.append(shape)
  if k=='track':
 ...
 for layer,width,points in ROUTES:
  ...
  assert any(t.distance(Point(*points[-1]))<1e-6 for t in existing_vias)
...
def run_drc():
 work=ROOT/'hardware/kicad/native/_p2_vcore_u1_49_041';shutil.rmtree(work,ignore_errors=True)
 ...
 try:
  ...
  (OUT/'PCB-MAIN_P2_VCORE_U1_49_041_CANDIDATE_REV_A.kicad_pro').write_text(overlay)
...
def main():
 if '--check' in sys.argv:
  ...
  print('PCB-MAIN VCORE U1.49 041: PASS',record['drc']);return
 gate.deps();OUT.mkdir(parents=True,exist_ok=True);CANDIDATE.write_text(build())
 drc=run_drc();summary={'schema':'dioneya-pcb-main-vcore-u1-49-041-v1','base_sha256':BASE_SHA,
  'candidate_sha256':sha(CANDIDATE),'routes':len(ROUTES),'track_segments':1,
  'via_size_drill_mm':[.25,.15],'total_length_mm':3.241,
  'vcore_supply_pi_and_return_review_b':'OPEN','filled_capped_via_in_pad_dfm_review':'OPEN',
