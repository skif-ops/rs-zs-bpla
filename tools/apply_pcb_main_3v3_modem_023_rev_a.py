#!/usr/bin/env python3
"""Join C35 to R33 on 3V3 over the modem reference over the In1.Cu GND_MODEM region."""
...
BASE = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-3V3-MODEM-022/PCB-MAIN_P2_3V3_MODEM_022_CANDIDATE_REV_A.kicad_pcb"
BASE_DRC = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-3V3-MODEM-022/drc_candidate.json"
OUT = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-3V3-MODEM-023"
CANDIDATE = OUT / "PCB-MAIN_P2_3V3_MODEM_023_CANDIDATE_REV_A.kicad_pcb"
BASE_SHA = "a47c8a875108ea392cc59cdef1fe51ffecbabfc9cf2ede00695ac2d622f2b2ca"
NAMESPACE = uuid.UUID("890b082f-8797-40e7-b1b7-56db72388db7")

# Net, width (mm), pad centre, bend and U1 pad centre (mm).
ROUTES = (
    ("3V3_DIGITAL", .25, ((18.675,34.25),(17.975,34.85),(17.175,34.85),(16.975,34.6),(16.925,34.25))),
)
...
def build() -> str:
    ...
    for net, width, points in ROUTES:
        for j,(a,b) in enumerate(zip(points,points[1:])):
            key = f"P2-3V3-MODEM-023|{net}|{width}|{j}|{a}|{b}"
...
def run_drc() -> dict:
    work = ROOT / "hardware/kicad/native/_p2_3v3_modem_023"
    ...
    try:
        ...
        (OUT / "PCB-MAIN_P2_3V3_MODEM_023_CANDIDATE_REV_A.kicad_pro").write_text(overlay, encoding="utf-8")
...
def main() -> None:
    if "--check" in sys.argv:
        ...
        print("PCB-MAIN 3V3 modem zone 023: PASS", record["drc"])
        return
    gate.deps()
    OUT.mkdir(parents=True, exist_ok=True)
    CANDIDATE.write_text(build(), encoding="utf-8")
    drc = run_drc()
    summary = {"schema": "dioneya-pcb-main-3v3-modem-023-v1", "base_sha256": BASE_SHA,
               "candidate_sha256": sha(CANDIDATE), "track_segments": 4, "width_mm": .25,
               "length_mm": 2.4, "drc": drc, "candidate_only_usb_u1_via_rules": True,
               "power_return_review_b": "OPEN: C35/R33 3V3 over GND_MODEM",
