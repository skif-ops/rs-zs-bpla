#!/usr/bin/env python3
"""Join two SIM_MUX_SEL pad islands around U13 on F.Cu."""
...
BASE = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-AAD-018/PCB-MAIN_P2_AAD_018_CANDIDATE_REV_A.kicad_pcb"
BASE_DRC = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-AAD-018/drc_candidate.json"
OUT = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-SIM-MUX-019"
CANDIDATE = OUT / "PCB-MAIN_P2_SIM_MUX_019_CANDIDATE_REV_A.kicad_pcb"
BASE_SHA = "0860bf139270c963d6790a4a4ddb19a7b461f0c2cfe42ef0d79a462fa2dd7363"
NAMESPACE = uuid.UUID("190606a9-5c49-482f-ac9a-b85d453c276b")

# Net, width (mm), U13/R45 pad centres and legal bends (mm).
ROUTES = (
    ("SIM_MUX_SEL", .2, ((39.4,14.425),(36.825,13.0))),
    ("SIM_MUX_SEL", .2, ((39.4,20.925),(40.9,20.825),(41.2,20.425),(42.65,17.675),
                          (42.65,17.225),(40.3,14.525),(39.4,14.425))),
)
...
def build() -> str:
    ...
    for net, width, points in ROUTES:
        for j,(a,b) in enumerate(zip(points,points[1:])):
            key = f"P2-SIM-MUX-019|{net}|{width}|{j}|{a}|{b}"
...
def run_drc() -> dict:
    work = ROOT / "hardware/kicad/native/_p2_sim_mux_019"
    ...
    try:
        ...
        (OUT / "PCB-MAIN_P2_SIM_MUX_019_CANDIDATE_REV_A.kicad_pro").write_text(overlay, encoding="utf-8")
...
def main() -> None:
    if "--check" in sys.argv:
        ...
        print("PCB-MAIN SIM MUX 019: PASS", record["drc"])
        return
    gate.deps()
    ...
    drc = run_drc()
    from shapely.geometry import LineString
    summary = {"schema": "dioneya-pcb-main-sim-mux-019-v1", "base_sha256": BASE_SHA,
