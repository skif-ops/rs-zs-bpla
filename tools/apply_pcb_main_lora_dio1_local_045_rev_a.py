#!/usr/bin/env python3
"""Connect R65.1 to U10.13 on LORA_DIO1 across an inner-layer channel."""
...
BASE=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-I2C-SDA-TP-044/PCB-MAIN_P2_I2C_SDA_TP_044_CANDIDATE_REV_A.kicad_pcb'
BASE_DRC=BASE.parent/'drc_candidate.json'
OUT=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-LORA-DIO1-LOCAL-045'
CANDIDATE=OUT/'PCB-MAIN_P2_LORA_DIO1_LOCAL_045_CANDIDATE_REV_A.kicad_pcb'
BASE_SHA='d3df48a62b6657b8fc7b569ae14bf251b0b09dea0922603b9528f2e86be20e86'
NAMESPACE=uuid.UUID('d5dd2f69-c7b4-4809-a568-4a1021bc69f4')
NET='LORA_DIO1'
ROUTES=(('F.Cu',.15,((66.55,48.27),(65.05,48.27))),
        ('In3.Cu',.15,((64.925,53.75),(65.025,52.95),(64.325,51.75),
                        (64.425,51.35),(64.825,50.85),(64.725,50.45),
                        (64.225,49.95),(64.125,49.45),(65.05,48.27))))
VIAS=((64.925,53.75),(65.05,48.27))
...
def check_geometry():
 ...
 pads=[];endpoints=set();inner_tracks=[]
 ...
 for n,k,l,shape,raw in geo.items(b):
  ...
  if k=='track' and l=='In3.Cu' and n==NET:inner_tracks.append(shape.buffer(raw.width/2,8))
  if k=='track':
 ...
 assert any(n==NET and l=='F.Cu' and shape.contains(Point(*ROUTES[0][2][0])) for n,l,shape in pads)
 assert any(n==NET and l=='F.Cu' and shape.contains(Point(*VIAS[0])) for n,l,shape in pads)
 assert ROUTES[0][2][-1]==VIAS[1]==ROUTES[1][2][-1]
 assert ROUTES[1][2][0]==VIAS[0]
 for x,y in VIAS:
  q=Point(x,y)
...
def run_drc():
 work=ROOT/'hardware/kicad/native/_p2_lora_dio1_local_045';shutil.rmtree(work,ignore_errors=True)
 ...
 try:
  ...
  (OUT/'PCB-MAIN_P2_LORA_DIO1_LOCAL_045_CANDIDATE_REV_A.kicad_pro').write_text(overlay)
...
def main():
 if '--check' in sys.argv:
  ...
  print('PCB-MAIN LORA DIO1 local 045: PASS',record['drc']);return
 gate.deps();OUT.mkdir(parents=True,exist_ok=True);CANDIDATE.write_text(build())
 drc=run_drc();summary={'schema':'dioneya-pcb-main-lora-dio1-local-045-v1','base_sha256':BASE_SHA,
  'candidate_sha256':sha(CANDIDATE),'routes':len(ROUTES),'track_segments':9,
  'via_size_drill_mm':[.25,.15],'total_length_mm':7.877,
  'lora_dio1_signal_return_review_b':'OPEN','small_via_dfm_review':'OPEN',
