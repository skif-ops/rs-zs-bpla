#!/usr/bin/env python3
"""Connect TP_EOL I2C2_SDA_BUS to its existing inner-layer run."""
...
BASE=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-PWR-GOOD-TP-043/PCB-MAIN_P2_PWR_GOOD_TP_043_CANDIDATE_REV_A.kicad_pcb'
BASE_DRC=BASE.parent/'drc_candidate.json'
OUT=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-I2C-SDA-TP-044'
CANDIDATE=OUT/'PCB-MAIN_P2_I2C_SDA_TP_044_CANDIDATE_REV_A.kicad_pcb'
BASE_SHA='f6560764d8dc97ab049d4ddcb4eb5427265ccd1bda7cae32ab6acd5bcb266578'
NAMESPACE=uuid.UUID('809b3ae6-973c-48f5-bc6e-3a14817cc81a')
NET='I2C2_SDA_BUS'
ROUTES=(('B.Cu',.15,((63.48,23),(63.23,24.75))),
        ('In3.Cu',.15,((63.23,24.75),(63.43,25.25),(62.833,30.0044))))
VIAS=((63.23,24.75),)
...
def run_drc():
 work=ROOT/'hardware/kicad/native/_p2_i2c_sda_tp_044';shutil.rmtree(work,ignore_errors=True)
 ...
 try:
  ...
  (OUT/'PCB-MAIN_P2_I2C_SDA_TP_044_CANDIDATE_REV_A.kicad_pro').write_text(overlay)
...
def main():
 if '--check' in sys.argv:
  ...
  print('PCB-MAIN I2C SDA TP 044: PASS',record['drc']);return
 gate.deps();OUT.mkdir(parents=True,exist_ok=True);CANDIDATE.write_text(build())
 drc=run_drc();summary={'schema':'dioneya-pcb-main-i2c-sda-tp-044-v1','base_sha256':BASE_SHA,
  'candidate_sha256':sha(CANDIDATE),'routes':len(ROUTES),'track_segments':3,
  'via_size_drill_mm':[.25,.15],'total_length_mm':7.098,
  'i2c_rise_time_and_return_review_b':'OPEN','small_via_dfm_review':'OPEN',
