#!/usr/bin/env python3
"""Connect R62.1 GNSS switched-bias pad to its existing back-side run."""
...
BASE=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-GNSS-BIAS-R61-047/PCB-MAIN_P2_GNSS_BIAS_R61_047_CANDIDATE_REV_A.kicad_pcb'
BASE_DRC=BASE.parent/'drc_candidate.json'
OUT=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-GNSS-SWITCHED-R62-048'
CANDIDATE=OUT/'PCB-MAIN_P2_GNSS_SWITCHED_R62_048_CANDIDATE_REV_A.kicad_pcb'
BASE_SHA='6658aedbf02d79bfaa29777459f0da5b21cde580fa01407d7519dd88f1ab6f21'
NAMESPACE=uuid.UUID('ba582dba-a6e4-4a70-aa5f-d39804692b4a')
NET='GNSS_ANT_SWITCHED'
ROUTES=(('B.Cu',.3,((52.8,51.25),(53.5,51.25))),)
VIAS=((52.8,51.25),)
...
def check_geometry():
 ...
 assert any(shape.distance(Point(*ROUTES[0][2][-1]))<.01 for shape in back_tracks)
 assert ROUTES[0][2][0]==VIAS[0]
 for x,y in VIAS:
  q=Point(x,y)
...
def run_drc():
 work=ROOT/'hardware/kicad/native/_p2_gnss_switched_r62_048';shutil.rmtree(work,ignore_errors=True)
 ...
 try:
  ...
  (OUT/'PCB-MAIN_P2_GNSS_SWITCHED_R62_048_CANDIDATE_REV_A.kicad_pro').write_text(overlay)
...
def main():
 if '--check' in sys.argv:
  ...
  print('PCB-MAIN GNSS switched R62 048: PASS',record['drc']);return
 gate.deps();OUT.mkdir(parents=True,exist_ok=True);CANDIDATE.write_text(build())
 drc=run_drc();summary={'schema':'dioneya-pcb-main-gnss-switched-r62-048-v1','base_sha256':BASE_SHA,
  'candidate_sha256':sha(CANDIDATE),'routes':len(ROUTES),'track_segments':1,
  'via_size_drill_mm':[.25,.15],'total_length_mm':.7,
  'gnss_bias_and_rf_coupling_review_b':'OPEN','via_in_pad_dfm_review':'OPEN',
