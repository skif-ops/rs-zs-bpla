#!/usr/bin/env python3
"""Join I2C2_SCL_BUS between the U3-side F.Cu track and U4-side via on In3.Cu."""
...
BASE=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-U1-11-034/PCB-MAIN_P2_U1_11_034_CANDIDATE_REV_A.kicad_pcb'
BASE_DRC=BASE.parent/'drc_candidate.json'
OUT=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-I2C-SCL-035'
CANDIDATE=OUT/'PCB-MAIN_P2_I2C_SCL_035_CANDIDATE_REV_A.kicad_pcb'
BASE_SHA='4b9fd52bdc7d3c94b3f0940800c67cd88ea85e3408c69abe1e5b296a39e4b6c1'
NAMESPACE=uuid.UUID('971e2713-1d2e-4b5f-86dd-b27a682c19b0')
NET='I2C2_SCL_BUS'
ROUTES=(('In3.Cu',.15,((65.2375,29.2552),(65.25,29.25),
                        (66.35,30.1),(66.6,30.35),(66.65,30.45),
                        (66.65,31.3),(66.25,31.7),(66.15,32.0),
                        (65.65,34.85),(65.7,38.15),(65.6851,38.1701))),)
VIAS=((65.2375,29.2552),)
...
def check_geometry():
 ...
 pads=set();endpoints=set();existing_vias=set();front_tracks=[]
 ...
 for n,k,l,shape,raw in geo.items(b):
  if k=='via':
   ...
   existing_vias.add((n,round(shape.x,4),round(shape.y,4)))
  if k=='track' and l=='F.Cu' and n==NET:front_tracks.append(shape)
  if k=='track':
 ...
 for layer,width,points in ROUTES:
  ...
  for x,y in (points[0],points[-1]):
   assert (NET,layer,x,y) in pads or (NET,layer,x,y) in endpoints or (x,y) in VIAS or (NET,x,y) in existing_vias
  assert not [(n,line.distance(c)-r-width/2) for c,r,n in holes if n!=NET and line.distance(c)-r-width/2<.25-1e-6]
 for x,y in VIAS:
  assert any(t.distance(Point(x,y))<.001 for t in front_tracks)
...
def run_drc():
 work=ROOT/'hardware/kicad/native/_p2_i2c_scl_035';shutil.rmtree(work,ignore_errors=True)
 ...
 try:
  ...
  (OUT/'PCB-MAIN_P2_I2C_SCL_035_CANDIDATE_REV_A.kicad_pro').write_text(overlay)
...
def main():
 if '--check' in sys.argv:
  ...
  print('PCB-MAIN I2C SCL 035: PASS',record['drc']);return
 gate.deps();OUT.mkdir(parents=True,exist_ok=True);CANDIDATE.write_text(build())
 drc=run_drc();summary={'schema':'dioneya-pcb-main-i2c-scl-035-v1','base_sha256':BASE_SHA,
  'candidate_sha256':sha(CANDIDATE),'routes':len(ROUTES),'track_segments':10,
  ...
  'i2c_rise_time_return_review_b':'OPEN','small_via_dfm_review':'OPEN',
