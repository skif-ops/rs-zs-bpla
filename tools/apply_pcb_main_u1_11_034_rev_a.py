#!/usr/bin/env python3
"""Escape U1.11 3V3 with a candidate-process via-in-pad to the C7 supply via."""
...
BASE=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-C7-3V3-033/PCB-MAIN_P2_C7_3V3_033_CANDIDATE_REV_A.kicad_pcb'
BASE_DRC=BASE.parent/'drc_candidate.json'
OUT=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-U1-11-034'
CANDIDATE=OUT/'PCB-MAIN_P2_U1_11_034_CANDIDATE_REV_A.kicad_pcb'
BASE_SHA='d28f9952617e0ce7e03db99e01ac304fe896b9b9603205d3e700129621543f24'
NAMESPACE=uuid.UUID('f3025ec5-c61a-4628-a083-ebcafb227812')
NET='3V3_DIGITAL'
ROUTES=(('In3.Cu',.30,((44.25,29.0),(44.25,26.5))),)
VIAS=((44.25,29.0),)
...
def check_geometry():
 ...
 for x,y in VIAS:
  assert (NET,'F.Cu',x,y) in pads or any(t.distance(Point(x,y))<.001 for t in front_tracks)
...
def run_drc():
 work=ROOT/'hardware/kicad/native/_p2_u1_11_034';shutil.rmtree(work,ignore_errors=True)
 ...
 try:
  ...
  (OUT/'PCB-MAIN_P2_U1_11_034_CANDIDATE_REV_A.kicad_pro').write_text(overlay)
...
def main():
 if '--check' in sys.argv:
  ...
  print('PCB-MAIN U1 3V3 034: PASS',record['drc']);return
 gate.deps();OUT.mkdir(parents=True,exist_ok=True);CANDIDATE.write_text(build())
 drc=run_drc();summary={'schema':'dioneya-pcb-main-u1-11-034-v1','base_sha256':BASE_SHA,
  ...
  'u1_power_integrity_review_b':'OPEN','via_in_pad_dfm_review':'OPEN',
