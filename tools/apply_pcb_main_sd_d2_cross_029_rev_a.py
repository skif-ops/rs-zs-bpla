#!/usr/bin/env python3
"""Bridge SD_D2_CARD across SD_D3_CARD with two candidate-process vias."""
...
BASE=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-SWD-3V3-BACK-028/PCB-MAIN_P2_SWD_3V3_BACK_028_CANDIDATE_REV_A.kicad_pcb'
BASE_DRC=BASE.parent/'drc_candidate.json'
OUT=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-SD-D2-029'
CANDIDATE=OUT/'PCB-MAIN_P2_SD_D2_029_CANDIDATE_REV_A.kicad_pcb'
BASE_SHA='f0e1243eb99f239ea77058ad9e5b62e1e045d696a0474ca938b593c96021cf2e'
NAMESPACE=uuid.UUID('b674527f-a439-4969-859d-c7d3c1c12110')
NET='SD_D2_CARD'
ROUTES=(('F.Cu',.15,((90.325,24.25),(90.625,23.95))),
        ('In3.Cu',.15,((90.325,23.0),(90.625,23.95))))
VIAS=((90.325,23.0),(90.625,23.95))
...
def check_geometry():
 ...
 for x,y in VIAS:
  assert (NET,'F.Cu',x,y) in pads or (x,y)==VIAS[-1]
...
def run_drc():
 work=ROOT/'hardware/kicad/native/_p2_sd_d2_029';shutil.rmtree(work,ignore_errors=True)
 ...
 try:
  ...
  (OUT/'PCB-MAIN_P2_SD_D2_029_CANDIDATE_REV_A.kicad_pro').write_text(overlay)
...
def main():
 if '--check' in sys.argv:
  ...
  print('PCB-MAIN SD D2 029: PASS',record['drc']);return
 gate.deps();OUT.mkdir(parents=True,exist_ok=True);CANDIDATE.write_text(build())
 drc=run_drc();summary={'schema':'dioneya-pcb-main-sd-d2-029-v1','base_sha256':BASE_SHA,
  'candidate_sha256':sha(CANDIDATE),'routes':len(ROUTES),'track_segments':2,
  'via_size_drill_mm':[.25,.15],'total_length_mm':1.421,
