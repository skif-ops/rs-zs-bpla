#!/usr/bin/env python3
"""Connect the PWR_GOOD test pad to its existing inner-layer run."""
...
BASE=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-TP-3V3-WEST-042/PCB-MAIN_P2_TP_3V3_WEST_042_CANDIDATE_REV_A.kicad_pcb'
BASE_DRC=BASE.parent/'drc_candidate.json'
OUT=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-PWR-GOOD-TP-043'
CANDIDATE=OUT/'PCB-MAIN_P2_PWR_GOOD_TP_043_CANDIDATE_REV_A.kicad_pcb'
BASE_SHA='483f78aed9594b13663db172bd7067258a186558d962562a4e81b705973f3fc4'
NAMESPACE=uuid.UUID('7aa719c3-a7be-4d0b-aad7-048ecdc23425')
NET='PWR_GOOD'
ROUTES=(('B.Cu',.15,((48.24,23),(48.24,24.75))),
        ('In3.Cu',.15,((48.24,24.75),(50.54,21.75),(50.9,18.6248))))
VIAS=((48.24,24.75),)
...
def check_geometry():
 ...
 assert any(n==NET and l=='B.Cu' and shape.contains(Point(*ROUTES[0][2][0])) for n,l,shape in pads)
 assert ROUTES[0][2][-1]==VIAS[0]==ROUTES[1][2][0]
 assert any(shape.distance(Point(*ROUTES[1][2][-1]))<1e-6 for shape in inner_tracks)
 for x,y in VIAS:
  q=Point(x,y)
...
def run_drc():
 work=ROOT/'hardware/kicad/native/_p2_pwr_good_tp_043';shutil.rmtree(work,ignore_errors=True)
 ...
 try:
  ...
  (OUT/'PCB-MAIN_P2_PWR_GOOD_TP_043_CANDIDATE_REV_A.kicad_pro').write_text(overlay)
...
def main():
 if '--check' in sys.argv:
  ...
  print('PCB-MAIN PWR_GOOD TP 043: PASS',record['drc']);return
 gate.deps();OUT.mkdir(parents=True,exist_ok=True);CANDIDATE.write_text(build())
 drc=run_drc();summary={'schema':'dioneya-pcb-main-pwr-good-tp-043-v1','base_sha256':BASE_SHA,
  'candidate_sha256':sha(CANDIDATE),'routes':len(ROUTES),'track_segments':3,
  'via_size_drill_mm':[.25,.15],'total_length_mm':8.676,
  'pwr_good_mixed_domain_return_review_b':'OPEN','small_via_dfm_review':'OPEN',
