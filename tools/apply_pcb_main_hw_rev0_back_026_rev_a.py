#!/usr/bin/env python3
"""Experimental HW_REV0 route on B.Cu with two standard through vias."""
...
BASE = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-X1-FEED-025/PCB-MAIN_P2_X1_FEED_025_CANDIDATE_REV_A.kicad_pcb"
BASE_DRC = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-X1-FEED-025/drc_candidate.json"
OUT = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-HW-REV0-BACK-026"
CANDIDATE = OUT / "PCB-MAIN_P2_HW_REV0_BACK_026_CANDIDATE_REV_A.kicad_pcb"
BASE_SHA = "f1b2e2b43fe02e7aed5b1b9cc10932920a3a59526f4e970bf7622ed216dacd0a"
NAMESPACE = uuid.UUID("306a0886-91ce-4792-9e22-7c29176a126e")

NET = "HW_REV0"
WIDTH = .15
VIA_A, VIA_B = (57.675, 20.15), (61.025, 27.05)
FRONT_STUBS = (((57.075, 20.75), VIA_A), (VIA_B, (61.425, 27.25)))
INNER_PATH = (VIA_A, (62.775, 20.15), (63.975, 20.55), (65.675, 21.75),
              (65.775, 22.25), (62.875, 27.25), (61.675, 27.35), VIA_B)
...
def copper_clearance_check() -> None:
    ...
    clear("B.Cu", inner.buffer(WIDTH / 2 + .2, 8))
...
def build() -> str:
    ...
    segments = []
    for layer, points in (("F.Cu", FRONT_STUBS[0]), ("B.Cu", INNER_PATH),
                          ("F.Cu", FRONT_STUBS[1])):
        for j,(a,b) in enumerate(zip(points,points[1:])):
            key = f"P2-HW-REV0-BACK-026|{NET}|{layer}|{WIDTH}|{j}|{a}|{b}"
    ...
    for j, (x, y) in enumerate((VIA_A, VIA_B)):
        key = f"P2-HW-REV0-BACK-026|{NET}|via|{j}|{x}|{y}"
...
def run_drc() -> dict:
    work = ROOT / "hardware/kicad/native/_p2_hw_rev0_back_026"
    ...
    try:
        ...
        (OUT / "PCB-MAIN_P2_HW_REV0_BACK_026_CANDIDATE_REV_A.kicad_pro").write_text(overlay, encoding="utf-8")
...
def main() -> None:
    if "--check" in sys.argv:
        ...
        print("PCB-MAIN HW_REV0 B.Cu 026: PASS", record["drc"])
        return
    gate.deps()
    ...
    summary = {"schema": "dioneya-pcb-main-hw-rev0-back-026-v1", "base_sha256": BASE_SHA,
               ...
               "return_review_b": "OPEN: GND_DIGITAL stitching near second via, 1.505 mm",
