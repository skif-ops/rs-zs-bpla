...
def main() -> None:
    ...
    require(u5["3"]["RevA_Net"] == "3V3_DIGITAL", "TPS7A2018 EN must follow 3V3 in Rev.A")
    ...
    for stale in ("10-contact MAIN/PWR contract", "INA226_without_I2C", "ALERT_only"):
        require(stale not in combined, f"stale PCB-PWR capture token present: {stale}")

    print("PCB-PWR Rev.A pin/net capture authority audit PASS")
