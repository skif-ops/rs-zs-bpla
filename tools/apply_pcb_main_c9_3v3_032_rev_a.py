#!/usr/bin/env python3
"""Join the C9 3V3 capacitor island to the R14/C6 supply via on In3.Cu."""
...
BASE=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-SD-D1-031/PCB-MAIN_P2_SD_D1_031_CANDIDATE_REV_A.kicad_pcb'
BASE_DRC=BASE.parent/'drc_candidate.json'
OUT=ROOT/'hardware/kicad/candidates/PCB-ROUTING-P2-C9-3V3-032'
CANDIDATE=OUT/'PCB-MAIN_P2_C9_3V3_032_CANDIDATE_REV_A.kicad_pcb'
BASE_SHA='c27f98735cf06d948fffd85db963ab71c4568df58092e693db6167f3212bb099'
NAMESPACE=uuid.UUID('ceef14ad-27f0-489b-b92d-8a6ae1ae4567')
NET='3V3_DIGITAL'
ROUTES=(('In3.Cu',.30,((40.8768,35.7033),(40.175,31.7))),)
VIAS=((40.8768,35.7033),)
...
def run_drc():
 work=ROOT/'hardware/kicad/native/_p2_c9_3v3_032';shutil.rmtree(work,ignore_errors=True)
 ...
 try:
  ...
  (OUT/'PCB-MAIN_P2_C9_3V3_032_CANDIDATE_REV_A.kicad_pro').write_text(overlay)
...
def main():
 if '--check' in sys.argv:
  ...
  print('PCB-MAIN C9 3V3 032: PASS',record['drc']);return
 gate.deps();OUT.mkdir(parents=True,exist_ok=True);CANDIDATE.write_text(build())
 drc=run_drc();summary={'schema':'dioneya-pcb-main-c9-3v3-032-v1','base_sha256':BASE_SHA,
  ...
  'capacitor_supply_review_b':'OPEN','small_via_dfm_review':'OPEN',
