#!/usr/bin/env python3
"""Join U1.100 to its local C5 decoupler with a short 0.25 mm F.Cu run."""
...
BASE = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-U1-C7-013/PCB-MAIN_P2_U1_C7_013_CANDIDATE_REV_A.kicad_pcb"
BASE_DRC = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-U1-C7-013/drc_candidate.json"
OUT = ROOT / "hardware/kicad/candidates/PCB-ROUTING-P2-U1-C5-014"
CANDIDATE = OUT / "PCB-MAIN_P2_U1_C5_014_CANDIDATE_REV_A.kicad_pcb"
BASE_SHA = "d18f20d2c18b9f35d2b8006a396c8d127fa2d0936c3601bd03cc94639dc7909b"
NAMESPACE = uuid.UUID("890b082f-8797-40e7-b1b7-56db72388db7")

# Net, width (mm), pad centre, bend and U1 pad centre (mm).
ROUTES = (
    ("3V3_DIGITAL", .25, ((45.675,20.75),(45.725,21.6),(46.0,22.25))),
)
...
def build() -> str:
    ...
    for net, width, points in ROUTES:
        for j,(a,b) in enumerate(zip(points,points[1:])):
            key = f"P2-U1-C5-014|{net}|{width}|{j}|{a}|{b}"
...
def run_drc() -> dict:
    work = ROOT / "hardware/kicad/native/_p2_u1_c5_014"
    ...
    try:
        ...
        (OUT / "PCB-MAIN_P2_U1_C5_014_CANDIDATE_REV_A.kicad_pro").write_text(overlay, encoding="utf-8")
...
def main() -> None:
    if "--check" in sys.argv:
        ...
        print("PCB-MAIN U1 C5 power 014: PASS", record["drc"])
        return
    gate.deps()
    OUT.mkdir(parents=True, exist_ok=True)
    CANDIDATE.write_text(build(), encoding="utf-8")
    drc = run_drc()
    summary = {"schema": "dioneya-pcb-main-u1-c5-power-014-v1", "base_sha256": BASE_SHA,
               "candidate_sha256": sha(CANDIDATE), "track_segments": 2, "width_mm": .25,
               "length_mm": 1.56, "drc": drc, "candidate_only_usb_u1_via_rules": True,
