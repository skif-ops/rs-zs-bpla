# DIONEA-INTEGRATION-v0.14

Дата: 17.09.2026.

Статус: `OPEN / INTEGRATION BASELINE / NOT FOR MANUFACTURE`.

Версия синхронизирует общий интеграционный baseline `develop` и `main` с
проверенным срезом `evt-pre-20`. Поставочные области `evt-mb` и `evt-pre-20`
не объединялись и не копировались друг в друга.

## Исходные контрольные точки

| Область | Ветка | SHA |
|---|---|---|
| Предыдущий интеграционный baseline v0.13 | `main` / `develop` | `bb7ff98d68075b3a606cfa3cfa1a5fde14d43e29` |
| Макетный EVT-MB v1.3, без изменений | `evt-mb` | `7d0f7f182e342cd3a3c6ba6f28bc6f138c71c4e1` |
| Предсерийный EVT-PRE-20 Rev.A | `evt-pre-20` | `7ed5b021b1e7eddfda616b54fbc79e1bf0221702` |
...
- на данном этапе зафиксирована одна модель каждого системного компонента по
  балансу цены и качества; решение о продолжении или замене принимается по EVT.
...
- `PCB-MAIN`: commit `9aceca9531f0b9c18679bee1a8050ae7cd94308a` и hierarchy PDF
  ...
  приняты решением `ACCEPT_HIERARCHY_ONLY`; routing, Review B, CAM/DFM и
  manufacture этим решением не разрешены;
- `PCB-PWR`: активный F1 — Littelfuse `0451008.MRL`; bounded value-only ECO
  применён без изменения footprint, placement, topology или nets. KiCad 9.0.9
  ERC имеет 0 нарушений на пяти листах, commit-bound PDF evidence прошло;
  независимый post-ECO hierarchy review остаётся открытым. Qualification — 1/20;
- `PCB-MIC`: copper-return subgate принят; общий Review B, CAM/DFM и
  manufacturing release остаются открыты;
...
4. Не повышать post-ECO hierarchy acceptance до routing/Review B/manufacture без
