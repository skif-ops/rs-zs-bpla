...
Файлы `config/lora/RU868.yaml` и `EU868.yaml` намеренно имеют пустые channel/power/duty-cycle поля и `tx_enabled: false`. Передача может быть включена только после проверки LoRa Alliance RP002-1.0.5, местных требований, conducted RF и антенны.

Старый символ firmware `zs_ru868_channels_hz={868.1,868.3,868.5,868.7 MHz}` не является утверждённым RU-профилем и запрещён для release. Он должен быть заменён загрузкой подписанного регионального профиля и fail-closed проверкой.
